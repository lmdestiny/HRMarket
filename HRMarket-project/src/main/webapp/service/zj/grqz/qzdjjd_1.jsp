...
$(function(){
 	$("#bip_citizenid").blur(function(){
		$.get("${pageContext.request.contextPath}/jobStopStart/regist",
				{"bipCitizenid":$("#bip_citizenid").val()},
				function(date){
					if(date.status ==200){
						$("#bip_name").val(date.data);
						$("#bip_name").attr("readonly",true);
					}else{
						alert(date.msg);
						$("#bip_name").val("");
						$("#bip_citizenid").val("");
	 ...
	 $("#bip_name").blur(function(){
		$.post("${pageContext.request.contextPath}/jobStopStart/regist2",
			{"bipName":$("#bip_name").val()},
			function(date){
				if(date.status ==200){
					$("#bip_citizenid").val(date.data);
					$("#bip_citizenid").attr("readonly",true);
				}else{
					alert(date.msg);
					$("#bip_name").val("");
					$("#bip_citizenid").val("");
...
<script>
	function dosubmit(){
		...
		form1.action="<%=request.getContextPath()%>/jobStopStart/getInfo";
		form1.submit();
	}
					...
					<table width="250" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td width="194"><input id="bip_citizenid"
								...
								style='width: 100%' onblur="if(check(form1))huitian1();"></td>
						</tr>
					</table>
				</TD>
			</TR>
		...
		<table width="98%" border="0" align="center">
			<TR align="center" class="line2">
				<TD><INPUT name="button" type="button" class="BUTTONs3"
					value="确 定" onClick="dosubmit()"> &nbsp;&nbsp; <INPUT
					class="BUTTONs3" type="reset" value="取 消" name="button2"></TD>
			</TR>
		</table>
	</form>
</body>
